...
const receipt = JSON.parse(fs.readFileSync(new URL("../public/receipts/ANTIMATTERIUM_ORG_PROFILE_MOVE55_CONTROL_V0225_BACKLINK.json", import.meta.url), "utf8"));
const pkg = JSON.parse(fs.readFileSync(new URL("../package.json", import.meta.url), "utf8"));

test("Org profile binds CONTROL v0.2.25 public closure", () => {
  assert.equal(pkg.name, "@antimatterium/org-profile");
  assert.equal(pkg.version, "0.0.34");
  assert.equal(pkg.scripts["verify:move55-control-v0225-backlink"], "node scripts/verify-org-profile-move55-control-v0225-backlink.mjs");
  assert.equal(pkg.scripts["test:move55-control-v0225-backlink"], "node --test tests/org-profile-move55-control-v0225-backlink.test.mjs");
  ...
  assert.equal(receipt.surface_version, "0.0.34");
  assert.equal(receipt.surface_tag, "v0.0.34-antimatterium-org-profile-control-v0225-backlink");
  ...
  console.log("ANTIMATTERIUM_ORG_PROFILE_MOVE55_CONTROL_V0225_BACKLINK_VERIFY_PASS=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_CONTROL_V0225_RELEASE_BOUND=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_CONTROL_V0225_MEMBER=true");
  console.log("ANTIMATTERIUM_SHORT_PUBLIC_TAG_REQUIRED=true");
  console.log("ANTIMATTERIUM_NO_LOCAL_ROOT_REQUIRED=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_MOVE55_CONTROL_V0225_BACKLINK_ID=" + receipt.backlink_id);

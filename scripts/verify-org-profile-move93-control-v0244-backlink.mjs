...
const receipt = JSON.parse(fs.readFileSync('public/receipts/ANTIMATTERIUM_ORG_PROFILE_MOVE93_CONTROL_V0244_BACKLINK.json', 'utf8'));
const { backlink, ...base } = receipt;
const expectedBacklinkId = crypto.createHash('sha256').update(JSON.stringify(base)).digest('hex');

assert.equal(receipt.schema, 'antimatterium.org_profile.move93.control_v0244_backlink.v1');
...
assert.equal(receipt.surface.title, 'Org profile');
assert.equal(receipt.surface.repository, 'ANTIMATTERIUM/.github');
assert.equal(receipt.surface.package_name, '@antimatterium/org-profile');
assert.equal(receipt.surface.version, '0.0.53');
assert.equal(receipt.surface.release_tag, 'v0.0.53-antimatterium-org-profile-control-v0244-backlink');
...
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE93_CONTROL_V0244_BACKLINK_VERIFY_PASS=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE93_CONTROL_V0244_BACKLINK_CONTROL_V0244_RELEASE_BOUND=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE93_CONTROL_V0244_BACKLINK_CONTROL_V0244_MEMBER=true');
console.log('ANTIMATTERIUM_SHORT_PUBLIC_TAG_REQUIRED=true');
console.log('ANTIMATTERIUM_NO_LOCAL_ROOT_REQUIRED=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE93_CONTROL_V0244_BACKLINK_ID=' + backlink.id);

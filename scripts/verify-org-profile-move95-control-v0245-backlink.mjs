...
const receipt = JSON.parse(fs.readFileSync('public/receipts/ANTIMATTERIUM_ORG_PROFILE_MOVE95_CONTROL_V0245_BACKLINK.json', 'utf8'));
const { backlink, ...base } = receipt;
const expectedBacklinkId = crypto.createHash('sha256').update(JSON.stringify(base)).digest('hex');

assert.equal(receipt.schema, 'antimatterium.org_profile.move95.control_v0245_backlink.v1');
...
assert.equal(receipt.surface.version, '0.0.54');
assert.equal(receipt.surface.release_tag, 'v0.0.54-antimatterium-org-profile-control-v0245-backlink');
...
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE95_CONTROL_V0245_BACKLINK_VERIFY_PASS=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE95_CONTROL_V0245_BACKLINK_CONTROL_V0245_RELEASE_BOUND=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE95_CONTROL_V0245_BACKLINK_CONTROL_V0245_MEMBER=true');
console.log('ANTIMATTERIUM_SHORT_PUBLIC_TAG_REQUIRED=true');
console.log('ANTIMATTERIUM_NO_LOCAL_ROOT_REQUIRED=true');
console.log('ANTIMATTERIUM_ORG_PROFILE_MOVE95_CONTROL_V0245_BACKLINK_ID=' + backlink.id);

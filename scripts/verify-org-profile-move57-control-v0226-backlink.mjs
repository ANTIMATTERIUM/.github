...
const receipt = JSON.parse(fs.readFileSync(new URL("../public/receipts/ANTIMATTERIUM_ORG_PROFILE_MOVE57_CONTROL_V0226_BACKLINK.json", import.meta.url), "utf8"));
const pkg = JSON.parse(fs.readFileSync(new URL("../package.json", import.meta.url), "utf8"));

test("Org profile binds CONTROL v0.2.26 public closure", () => {
  assert.equal(pkg.name, "@antimatterium/org-profile");
  assert.equal(pkg.version, "0.0.35");
  assert.equal(pkg.scripts["verify:move57-control-v0226-backlink"], "node scripts/verify-org-profile-move57-control-v0226-backlink.mjs");
  assert.equal(pkg.scripts["test:move57-control-v0226-backlink"], "node --test tests/org-profile-move57-control-v0226-backlink.test.mjs");
  ...
  assert.equal(receipt.surface_version, "0.0.35");
  assert.equal(receipt.surface_tag, "v0.0.35-antimatterium-org-profile-control-v0226-backlink");
  ...
  console.log("ANTIMATTERIUM_ORG_PROFILE_MOVE57_CONTROL_V0226_BACKLINK_VERIFY_PASS=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_CONTROL_V0226_RELEASE_BOUND=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_CONTROL_V0226_MEMBER=true");
  console.log("ANTIMATTERIUM_SHORT_PUBLIC_TAG_REQUIRED=true");
  console.log("ANTIMATTERIUM_NO_LOCAL_ROOT_REQUIRED=true");
  console.log("ANTIMATTERIUM_ORG_PROFILE_MOVE57_CONTROL_V0226_BACKLINK_ID=" + receipt.backlink_id);
